...
gridITH = function(mat){
  ...
  ith = sum(mat$W[mat$type == 'B'])/(sum(mat$W[mat$type == 'A']) + 
                                       sum(mat$W[mat$type == 'B']))
  list(ith = ith, mat = mat)
}

gridPlot = function(mat, x = 'a2unscaled', y = 'a1unscaled', xlim = NULL, ylim = NULL, ploidy = FALSE,
                    printMutations = FALSE, xlab = 'Major array intensity',
                    ylab = 'Minor array intensity', scaled = TRUE, types = FALSE,
                    ith = TRUE, printout = NULL, width = 500, height = 500){
  if (is.list(mat)) {#To allow mat being a matrix in a
    #larger list object.
    #(Required for mutations to print.)
    obj = mat
    mat = obj$mat 
  }
  if (scaled) alpha = obj$pars$alpha else alpha = mat$alphamax[1]
  
  if (!scaled){
    xlab = paste(xlab, ', unscaled', sep = '')
    ylab = paste(ylab, ', unscaled', sep = '')
  }
  ...
  for (j in 1:length(EEs)){
    lines(range(EEs), rep(EEs[j], 2), lty = 'dotted', col = 'grey20', lwd = .5)
    lines(rep(EEs[j], 2), range(EEs), lty = 'dotted', col = 'grey20', lwd = .5)
  }
  
  if (printMutations) points(x[obj$muts$segment], y[obj$muts$segment], 
                             pch = 18, cex = .6)  
  if (ith) mtext(paste('ITH:', round(obj$ith[1]*100), '%'), adj = 1)
  if (ploidy)   legend('topleft', pch = c('',''), legend = c(paste('Ploidy =', format(round(obj$ploidy,1),nsmall = 1)),paste('Cell fraction', round(alpha*100), '%')),
                       bty = 'n')
...
subclonalCNdist = function(mat, cut = 3, w = 1-exp(-mat$length/500000),
  ...
  xlim = c(0, quantile(mat$c2, probs = c(.95))*c(1.2))
...
start.alphamax.f = function(mat, maxCN = 24, maxlines = 30, xlim = NULL, ylim = NULL,
  ...
  abline(0,1, col = 'red')
  #  contour(sort(mat$a2,decreasing = FALSE), sort(mat$a1,decreasing = FALSE),sort(mat$a2,decreasing = FALSE))
  z <- kde2d(mat$a2, mat$a1, n=50)
  ...
  n = max(length(bbx), length(bby))
  if (length(bbx) < n) bbx = seq(E2[closest], by = -deltax, length.out = n)
  if (length(bby) < n) bbx = seq(E1[closest], by = -deltay, length.out = n)
  nxs = c(1:length(bbx))[(length(bbx)-n+1):length(bbx)] 
  nys = c(1:length(bby))[(length(bby)-n+1):length(bby)] 
